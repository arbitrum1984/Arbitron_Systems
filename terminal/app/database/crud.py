...
# --- FRED (Macro) ---
def upsert_fred_data(series_id: str, observations: list) -> None:
    """Bulk upsert FRED observations."""
    series_id = series_id.upper()
    with db.get_connection() as conn:
        # Instead of deleting all, we can use UPSERT logic, but delete-refresh is simpler for caching chunks
        # Deleting old data for this series to ensure clean state (or valid overwrites)
        ...
        conn.executemany(
            "INSERT INTO fred_observations (series_id, date, value, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)",
            data
        )
        conn.commit()

def get_fred_observations(series_id: str) -> list:
    """Get time-series for a metric."""
    with db.get_connection() as conn:
        rows = conn.execute(
            "SELECT date, value FROM fred_observations WHERE series_id = ? ORDER BY date ASC",
            (series_id.upper(),)
        ).fetchall()
        return [dict(row) for row in rows]

def get_fred_last_update(series_id: str) -> str:
    """Check staleness."""
    with db.get_connection() as conn:
        row = conn.execute(
            "SELECT updated_at FROM fred_observations WHERE series_id = ? LIMIT 1",
            (series_id.upper(),)
        ).fetchone()
        return row["updated_at"] if row else None

def get_saved_fred_series() -> list:
    """Get list of cached series IDs."""
    with db.get_connection() as conn:
        rows = conn.execute(
            "SELECT DISTINCT series_id FROM fred_observations ORDER BY series_id"
        ).fetchall()
...
# --- Company Facts (EDGAR) ---
def upsert_company_facts(ticker: str, facts: list) -> None:
    """Bulk insert or replace company facts.
    
    This function deletes existing facts for the ticker and inserts new ones
    to ensure we have a fresh snapshot.
    """
    ticker = ticker.upper()
    with db.get_connection() as conn:
        # Transactional update: clear old, insert new
        conn.execute("DELETE FROM company_facts WHERE ticker = ?", (ticker,))
        
        # Prepare data tuple list: (cik, ticker, tag, value, period, form, unit)
        data = [
            (
                f.get("cik"), 
                ticker, 
        ...
        conn.executemany(
            """INSERT INTO company_facts 
               (cik, ticker, tag, value, period, form, unit, updated_at) 
               VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)""",
            data
        )
        conn.commit()

def get_company_facts(ticker: str) -> list:
    """Retrieve all facts for a given ticker."""
    with db.get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM company_facts WHERE ticker = ? ORDER BY period DESC", 
            (ticker.upper(),)
        ).fetchall()
        return [dict(row) for row in rows]

def get_last_update_time(ticker: str) -> str:
    """Get the timestamp of the last update for this ticker."""
    with db.get_connection() as conn:
        row = conn.execute(
            "SELECT updated_at FROM company_facts WHERE ticker = ? LIMIT 1", 
            (ticker.upper(),)
        ).fetchone()
        return row["updated_at"] if row else None

def get_saved_tickers() -> list:
    """Get list of all tickers that have data in company_facts."""
    with db.get_connection() as conn:
        rows = conn.execute(
